...
enum GameWindowMessage CPP_11( : Int){
    ...
    GWM_CREATE, GWM_DESTROY, GWM_ACTIVATE, GWM_ENABLE, GWM_LEFT_DOWN,
    GWM_LEFT_UP, GWM_LEFT_DOUBLE_CLICK, GWM_LEFT_DRAG, GWM_MIDDLE_DOWN,
    GWM_MIDDLE_UP, GWM_MIDDLE_DOUBLE_CLICK, GWM_MIDDLE_DRAG, GWM_RIGHT_DOWN,
    GWM_RIGHT_UP, GWM_RIGHT_DOUBLE_CLICK, GWM_RIGHT_DRAG, GWM_MOUSE_ENTERING,
    GWM_MOUSE_LEAVING, GWM_WHEEL_UP, GWM_WHEEL_DOWN, GWM_CHAR,
    GWM_SCRIPT_CREATE,
    // note that GWM_MOUSE_POS is only actually propagated to windows if the
    // static sendMousePosMessages is set to true in the window manager file.
    // See the comment on the static declaration for additional info
    GWM_INPUT_FOCUS, GWM_MOUSE_POS, GWM_IME_CHAR, GWM_IME_STRING
...
enum {
  ...
  WIN_STATUS_SMOOTH_TEXT =
      0x00002000, // Window text will be drawn with smoothing
  WIN_STATUS_ONE_LINE =
      0x00004000, // Window text will be drawn on only one line
  WIN_STATUS_NO_FLUSH =
      0x00008000, // Window images will not be unloaded when window is hidden
  WIN_STATUS_SEE_THRU = 0x00010000, // Will not draw, but it NOT hidden ... does
                                    // not apply to children
  WIN_STATUS_RIGHT_CLICK = 0x00020000, // Window pays attention to right clicks
  WIN_STATUS_WRAP_CENTERED =
      0x00040000, // Text will be centered on each word wrap or \n
  WIN_STATUS_CHECK_LIKE =
      0x00080000, // Make push buttons "check-like" with dual state
  WIN_STATUS_HOTKEY_TEXT =
      0x00100000, // Make push buttons "check-like" with dual state
  WIN_STATUS_USE_OVERLAY_STATES =
      0x00200000, // Push buttons will use the global automatic rendering
                  // overlay for disabled, hilited, and pushed.
  WIN_STATUS_NOT_READY = 0x00400000, // A disabled button that is available --
                                     // but not yet (power charge, fire delay).
  WIN_STATUS_FLASHING = 0x00800000,  // Used for buttons that do cameo flashes.
  WIN_STATUS_ALWAYS_COLOR =
      0x01000000, // Never render these buttons using greyscale renderer when
                  // button disabled.
  WIN_STATUS_ON_MOUSE_DOWN = 0x02000000, // Pushbutton triggers on mouse down.
  WIN_STATUS_SHORTCUT_BUTTON =
      0x04000000, // Oh god... this is a total hack for shortcut buttons to
                  // handle rendering text top left corner... when you edit
                  // this, remember to edit WindowStatusNames[]
...
struct WindowMessageBoxData {
  GameWinMsgBoxFunc yesCallback; ///< Function pointer to the Yes Button
                                 ///< Callback
  ...
  GameWinMsgBoxFunc
      cancelCallback; ///< Function pointer to the Cancel Button Callback
};
...
class GameWindow : public MemoryPoolObject {
  MEMORY_POOL_GLUE_ABC(
      GameWindow) ///< this abstract class needs memory pool hooks
...
public:
  ...
  Int winEnable(
      Bool
          enable);          /**< enable/disable a window, a disbled
  ...
  Color winGetEnabledColor(Int index) {
    return m_instData.m_enabledDrawData[index].color;
  }
  Color winGetEnabledBorderColor(Int index) {
    return m_instData.m_enabledDrawData[index].borderColor;
  }
  ...
  const Image *winGetDisabledImage(Int index) {
    return m_instData.m_disabledDrawData[index].image;
  }
  Color winGetDisabledColor(Int index) {
    return m_instData.m_disabledDrawData[index].color;
  }
  Color winGetDisabledBorderColor(Int index) {
    return m_instData.m_disabledDrawData[index].borderColor;
  }
  ...
  const Image *winGetHiliteImage(Int index) {
    return m_instData.m_hiliteDrawData[index].image;
  }
  Color winGetHiliteColor(Int index) {
    return m_instData.m_hiliteDrawData[index].color;
  }
  Color winGetHiliteBorderColor(Int index) {
    return m_instData.m_hiliteDrawData[index].borderColor;
  }
  ...
  void
  winSetDrawOffset(Int x,
                   Int y); ///< set offset for drawing background image data
  ...
  Int getTooltipDelay() {
    return m_instData.m_tooltipDelay;
  } ///< get tooltip delay
  void setTooltipDelay(Int delay) {
    m_instData.m_tooltipDelay = delay;
  } ///< set tooltip delay
  ...
  Int winSetCallbacks(GameWinInputFunc input, GameWinDrawFunc draw,
                      GameWinTooltipFunc tooltip); ///< set draw, input, tooltip

  // pick correlation ---------------------------------------------------------
  Bool winPointInWindow(
      Int x,
      Int y); /**is point inside this window?
...
extern WindowMsgHandledType GameWinBlockInput(GameWindow *window,
                                              ...
                                              WindowMsgData mData1,
                                              WindowMsgData mData2);
extern void GameWinDefaultTooltip(GameWindow *window, WinInstanceData *instData,
                                  UnsignedInt mouse);
