 ...
 ** 07/01/02 KM Shader library integration 08/05/02 KM Texture class redesign
 *(revisited)
 *---------------------------------------------------------------------------------------------*
...
WW3DAssetManager::WW3DAssetManager(void)
    : PrototypeLoaders(PROTOLOADERS_VECTOR_SIZE),
      ...
      WW3D_Load_On_Demand(false), Activate_Fog_On_Load(false),
...
static void Log_Textures(bool inited, unsigned &total_count,
  ...
  for (ite.First(); !ite.Is_Done(); ite.Next()) {
    ...
    WWDEBUG_SAY(("%32s	%4d * %4d (%15s), init %d, size: %14s bytes, refs: %d",
                 tex->Get_Texture_Name().str(), desc.Width, desc.Height,
                 tex_format.str(), tex->Is_Initialized(), number.str(),
                 tex->Num_Refs()));
  }
}
...
void WW3DAssetManager::Log_Texture_Statistics() {
  ...
  WWDEBUG_SAY(("\n%d initialized textures, totalling %14s bytes\n",
               total_initialized_count, number.str()));
  ...
  WWDEBUG_SAY(
      ("\n%d un-initialized textures, totalling, totalling %14s bytes\n",
       total_uninitialized_count, number.str()));
  /*
          RenderObjIterator *
...
RenderObjClass *WW3DAssetManager::Create_Render_Obj(const char *name) {
  ...
  if (WW3D_Load_On_Demand &&
      proto == nullptr) { // If we didn't find one, try to load on demand
...
HAnimClass *WW3DAssetManager::Get_HAnim(const char *name) {
  ...
  if (WW3D_Load_On_Demand &&
      anim == nullptr) { // If we didn't find it, try to load on demand
...
HTreeClass *WW3DAssetManager::Get_HTree(const char *name) {
  ...
  if (WW3D_Load_On_Demand &&
      htree == nullptr) { // If we didn't find it, try to load on demand
  ...
  if (!tex) {
    if (type == TextureBaseClass::TEX_REGULAR) {
      tex = NEW_REF(TextureClass,
                    (lower_case_name, nullptr, mip_level_count, texture_format,
                     allow_compression, allow_reduction));
    } else if (type == TextureBaseClass::TEX_CUBEMAP) {
      tex = NEW_REF(CubeTextureClass,
                    (lower_case_name, nullptr, mip_level_count, texture_format,
                     allow_compression, allow_reduction));
    } else if (type == TextureBaseClass::TEX_VOLUME) {
      tex = NEW_REF(VolumeTextureClass,
                    (lower_case_name, nullptr, mip_level_count, texture_format,
                     allow_compression, allow_reduction));
    } else {
...
void WW3DAssetManager::Log_All_Textures(void) {
  ...
  for (ite.First(); !ite.Is_Done(); ite.Next()) {
    ...
    WWDEBUG_SAY(
        ("%4.4dkb %s%s", bytes / 1024, tmp.str(), t->Get_Texture_Name().str()));
  }
