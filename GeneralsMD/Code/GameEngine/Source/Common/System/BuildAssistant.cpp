																											 ...
																											 Player *thePlayer)
{
	...
	for( them = iter2->first(); them; them = iter2->next() )
	{
		...
		if (themFactoryExitWidth>0) {
			...
			checkHisExit = true;
			Real c = WWMath::CosTrig(them->getOrientation());
			Real s = WWMath::SinTrig(them->getOrientation());
																								 ...
																								 Player *playerToBuild )
{
	...
	Real radius = WWMath::SqrtOrigin(gi.getMajorRadius() * gi.getMajorRadius() + gi.getMinorRadius() * gi.getMinorRadius());
