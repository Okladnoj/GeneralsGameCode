...
static void quitCallback(void) {
  ...
  { // GIANT CRAPTACULAR HACK ALERT!!!!  On sytems with little memory, we skip
    // all normal exit code
    //		//and let Windows clean up the mess.  This reduces exit times
    ...
    //    TheMemoryPoolFactory->prepareForMinSpecShutDown();
  }
...
// TheSuperHackers @tweak Now prints version information in an optional version
// label. Originally this label does not exist in the Main Menu. It can be
// copied from the Options Menu.
static void initLabelVersion() {
  NameKeyType versionID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:LabelVersion");
  GameWindow *labelVersion =
      TheWindowManager->winGetWindowFromId(nullptr, versionID);
...
void MainMenuInit(WindowLayout *layout, void *userData) {
  ...
  worldBuilderID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonWorldBuilder");
  ...
  buttonChallengeID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonChallenge");
  ...
  buttonUSARecentSaveID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonUSARecentSave");
  buttonUSALoadGameID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonUSALoadGame");
  buttonGLARecentSaveID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonGLARecentSave");
  buttonGLALoadGameID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonGLALoadGame");
  buttonChinaRecentSaveID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonChinaRecentSave");
  buttonChinaLoadGameID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonChinaLoadGame");
  buttonSinglePlayerID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonSinglePlayer");
  buttonMultiPlayerID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonMultiplayer");
  buttonMultiBackID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonMultiBack");
  buttonSingleBackID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonSingleBack");
  buttonLoadReplayBackID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonLoadReplayBack");
  buttonReplayID = TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonReplay");
  buttonLoadReplayID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonLoadReplay");
  buttonLoadID = TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonLoadGame");
  buttonCreditsID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonCredits");
  ...
  buttonDiffBackID =
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:ButtonDiffBack");
  ...
  //	buttonTRAINING = TheWindowManager->winGetWindowFromId( parentMainMenu,
  //buttonTRAININGID );
  buttonChallenge =
      TheWindowManager->winGetWindowFromId(parentMainMenu, buttonChallengeID);
  ...
  dropDownWindows[DROPDOWN_SINGLE] = TheWindowManager->winGetWindowFromId(
      parentMainMenu, TheNameKeyGenerator->nameToKey("MainMenu.wnd:MapBorder"));
  dropDownWindows[DROPDOWN_MULTIPLAYER] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:MapBorder1"));
  dropDownWindows[DROPDOWN_MAIN] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:MapBorder2"));
  dropDownWindows[DROPDOWN_LOADREPLAY] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:MapBorder3"));
  dropDownWindows[DROPDOWN_DIFFICULTY] = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:MapBorder4"));
  ...
  buttonCampaign = TheWindowManager->gogoGadgetPushButton(
      parentMainMenu, WIN_STATUS_ENABLED, 25, 54, 180, 26, &instData, nullptr,
      TRUE);
#endif
  ...
  versionStr=%s, distribution=%d", gameProductID, gameVersionUniqueIDStr,
  gameDistributionID)); ptCheckForPatch( gameProductID, gameVersionUniqueIDStr,
  gameDistributionID, patchAvailableCallback, PTFalse, nullptr );
                  //ptCheckForPatch( productID, versionUniqueIDStr,
  distributionID, mapPackAvailableCallback, PTFalse, nullptr );
          }
  }
  ...
  */
  ...
  GameWindow *rule = TheWindowManager->winGetWindowFromId(
      parentMainMenu,
      TheNameKeyGenerator->nameToKey("MainMenu.wnd:MainMenuRuler"));
  if (rule)
    rule->winHide(TRUE);
...
void DoResolutionDialog() {
  ...
  resAcceptMenu = TheWindowManager->gogoMessageBox(
      ...
      TheGameText->fetch("GUI:Resolution"), resTimerString, nullptr, nullptr,
...
frameTicker is %d", timeStarted, time(nullptr) , currentTime));
}
*/
...
WindowMsgHandledType MainMenuInput(GameWindow *window, UnsignedInt msg,
  ...
  case GWM_MOUSE_POS: {
    ...
    // TheSuperHackers @tweak 20/08/2025 Show mouse and menu immediately when
    // shellmap is disabled.
...
WindowMsgHandledType MainMenuSystem(GameWindow *window, UnsignedInt msg,
  ...
  case GBM_MOUSE_LEAVING: {
    ...
    if (controlID == onlineID) {
      TheScriptEngine->signalUIInteract(
          TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_ONLINE_UNHIGHLIGHTED]);
    } else if (controlID == networkID) {
      TheScriptEngine->signalUIInteract(
          TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_NETWORK_UNHIGHLIGHTED]);
    } else if (controlID == optionsID) {
      TheScriptEngine->signalUIInteract(
          TheShellHookNames[SHELL_SCRIPT_HOOK_MAIN_MENU_OPTIONS_UNHIGHLIGHTED]);
    } else if (controlID == exitID) {
      TheScriptEngine->signalUIInteract(
  ...
  case GBM_SELECTED: {
    ...
    if (control == buttonCampaign) {
      ...
      // don't allow mouse click slop that occurs during transitions to unset
      // this flag
      if (TheTransitionHandler->isFinished() && controlID != buttonEasyID &&
          controlID != buttonMediumID && controlID != buttonHardID) {
        // this toggle must only be reset if one of these buttons have not been
        // pressed
        // ...the difficulty selection behavior must have a chance to act upon
        // this toggle
    ...
    if (controlID == buttonSinglePlayerID) {
      ...
      //("Unable to get save load menu layout.") ); 				saveLoadMenuLayout->runInit(
      //&layoutType ); 				saveLoadMenuLayout->hide( FALSE );
      ...
      */
      ...
      DEBUG_ASSERTCRASH(optLayout != nullptr,
      ...
      if (_spawnl(_P_NOWAIT, "WorldBuilderD.exe", "WorldBuilderD.exe",
                  nullptr) < 0)
        MessageBoxOk(TheGameText->fetch("GUI:WorldBuilder"),
                     TheGameText->fetch("GUI:WorldBuilderLoadFailed"), nullptr);
#else
      if (_spawnl(_P_NOWAIT, "WorldBuilder.exe", "WorldBuilder.exe", nullptr) <
          0)
        MessageBoxOk(TheGameText->fetch("GUI:WorldBuilder"),
                     TheGameText->fetch("GUI:WorldBuilderLoadFailed"), nullptr);
#endif
    ...
    /*			else if(controlID == buttonTRAININGID)
                            {
                                    ...
                                    TheTransitionHandler->remove("MainMenuFactionTraining",
    TRUE); GameWindow *win =
    TheWindowManager->winGetWindowFromId(parentMainMenu,
    TheNameKeyGenerator->nameToKey("MainMenu.wnd:WinFactionTraining")); if(win)
                                            win->winHide(TRUE);
                                    TheTransitionHandler->reverse("MainMenuSinglePlayerMenuBackTraining");
                                    TheTransitionHandler->setGroup("MainMenuDifficultyMenuTraining");
                                    campaignSelected = TRUE;
    ...
    */
      ...
      //				layout =
      //TheWindowManager->winCreateLayout( "Menus/DifficultySelect.wnd" );
      //				layout->runInit();
      //				layout->hide( FALSE );
      //				layout->bringForward();
