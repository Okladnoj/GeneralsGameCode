...
GameSpyLoginPreferences::GameSpyLoginPreferences() { loadFromIniFile(); }
...
Bool GameSpyLoginPreferences::loadFromIniFile() {
  if (rts::ClientInstance::getInstanceId() > 1u) {
    AsciiString fname;
    fname.format("GameSpyLogin_Instance%.2u.ini",
...
void WOLLoginMenuInit(WindowLayout *layout, void *userData) {
  ...
  GameWindow *esrbTitle = TheWindowManager->winGetWindowFromId(
      nullptr, NAMEKEY("GameSpyLoginProfile.wnd:StaticTextESRBTop"));
  GameWindow *esrbParent = TheWindowManager->winGetWindowFromId(
      nullptr, NAMEKEY("GameSpyLoginProfile.wnd:ParentESRB"));
  ...
  parentWOLLogin =
      TheWindowManager->winGetWindowFromId(nullptr, parentWOLLoginID);
  ...
  buttonCreateAccount =
      TheWindowManager->winGetWindowFromId(nullptr, buttonCreateAccountID);
  buttonUseAccount =
      TheWindowManager->winGetWindowFromId(nullptr, buttonUseAccountID);
  buttonDontUseAccount =
      TheWindowManager->winGetWindowFromId(nullptr, buttonDontUseAccountID);
  ...
  comboBoxEmail =
      TheWindowManager->winGetWindowFromId(nullptr, comboBoxEmailID);
  comboBoxLoginName =
      TheWindowManager->winGetWindowFromId(nullptr, comboBoxLoginNameID);
  textEntryLoginName =
      TheWindowManager->winGetWindowFromId(nullptr, textEntryLoginNameID);
  textEntryPassword =
      TheWindowManager->winGetWindowFromId(nullptr, textEntryPasswordID);
  checkBoxRememberPassword =
      TheWindowManager->winGetWindowFromId(nullptr, checkBoxRememberPasswordID);
  textEntryMonth =
  ...
  textEntryYear =
      TheWindowManager->winGetWindowFromId(nullptr, textEntryYearID);
  ...
  if (parentWOLLogin) {
    ...
    textEntryPassword =
        TheWindowManager->winGetWindowFromId(nullptr, textEntryPasswordID);
    checkBoxRememberPassword = TheWindowManager->winGetWindowFromId(
        nullptr, checkBoxRememberPasswordID);
  ...
  if (TheShell->isAnimFinished() && !buttonPushed &&
    ...
    if (!loggedInOK && TheGameSpyPeerMessageQueue->getResponse(resp)) {
      ...
      case PeerResponse::PEERRESPONSE_DISCONNECT: {
        ...
        DEBUG_LOG(("Tearing down GameSpy from "
...
static Bool isNickOkay(UnicodeString nick) {
  static const WideChar *legalIRCChars = L"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghij"
                                         L"klmnopqrstuvwxyz0123456789[]`_^{|}-";
...
#define DATE_BUFFER_SIZE 256
  ...
  GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, nullptr, "yyyy", dateBuffer,
                DATE_BUFFER_SIZE);
  ...
  GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, nullptr, "MM", dateBuffer,
                DATE_BUFFER_SIZE);
  ...
  GetDateFormat(LOCALE_SYSTEM_DEFAULT, 0, nullptr, "dd", dateBuffer,
                DATE_BUFFER_SIZE);
  sysVal = atoi(dateBuffer);
      ...
      if (GameSpyUseProfiles) {
       ...
       // actually attempt to create an account based on info entered
        ...
        if (!email.isEmpty() && !login.isEmpty() && !password.isEmpty()) {
          ...
          strlcpy(req.arg.login.nick, login.str(),
                  ARRAY_SIZE(req.arg.login.nick));
          strlcpy(req.arg.login.email, email.str(),
                  ARRAY_SIZE(req.arg.login.email));
          strlcpy(req.arg.login.password, password.str(),
                  ARRAY_SIZE(req.arg.login.password));
