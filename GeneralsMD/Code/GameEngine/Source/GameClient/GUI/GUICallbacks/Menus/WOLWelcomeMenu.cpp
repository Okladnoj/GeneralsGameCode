...
static void updateNumPlayersOnline(void) {
  ...
  if (listboxInfo && TheGameSpyInfo) {
    ...
    // Removed number of players from string, and removed the argument. The
    // number is incorrect anyways... This was a Harvard initiated fix.
    ...
    GadgetListBoxAddEntryText(listboxInfo, L" ",
                              GameSpyColor[GSCOLOR_MOTD_HEADING], -1, -1);
...
void HandleOverallStats(const char *szHTTPStats, unsigned len) {
  ...
  if (!pToday) { // error
    DEBUG_LOG(
        ("Unable to parse win/loss stats.  Could not find 'Today' in:\n%s",
         szHTTPStats));
    return;
  }
  s_winStats.clear();
  ...
  for (int i = 0; i < ThePlayerTemplateStore->getPlayerTemplateCount();
       i++) { // get current side (USA, GLA, etc.)
    const PlayerTemplate *pTemplate =
        ThePlayerTemplateStore->getNthPlayerTemplate(i);
    if (!pTemplate->isPlayableSide() ||
        pTemplate->getSide().compare("Boss") == 0)
    ...
    if (pSide == nullptr) { // error, skip this side
    ...
    // x		DEBUG_LOG(("Added win percent: %s, %d", side.str(),
    // percent));
  }
}
...
void UpdateLocalPlayerStats(void) {
  ...
  GameWindow *welcomeParent = TheWindowManager->winGetWindowFromId(
      nullptr, NAMEKEY("WOLWelcomeMenu.wnd:WOLWelcomeMenuParent"));
...
void WOLWelcomeMenuInit(WindowLayout *layout, void *userData) {
  ...
  parentWOLWelcomeID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:WOLWelcomeMenuParent");
  buttonBackID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:ButtonBack");
  parentWOLWelcome =
      TheWindowManager->winGetWindowFromId(nullptr, parentWOLWelcomeID);
  ...
  buttonbuttonOptions =
      TheWindowManager->winGetWindowFromId(nullptr, buttonOptionsID);
  listboxInfoID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:InfoListbox");
  ...
  */
  ...
  buttonQuickMatchID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:ButtonQuickMatch");
  buttonQuickMatch = TheWindowManager->winGetWindowFromId(parentWOLWelcome,
                                                          buttonQuickMatchID);

  buttonLobbyID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:ButtonCustomMatch");
  buttonLobby =
      TheWindowManager->winGetWindowFromId(parentWOLWelcome, buttonLobbyID);

  buttonBuddiesID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:ButtonBuddies");
  buttonBuddies =
      TheWindowManager->winGetWindowFromId(parentWOLWelcome, buttonBuddiesID);

  buttonMyInfoID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:ButtonMyInfo");
  buttonMyInfo =
      TheWindowManager->winGetWindowFromId(parentWOLWelcome, buttonMyInfoID);

  buttonLadderID =
      TheNameKeyGenerator->nameToKey("WOLWelcomeMenu.wnd:ButtonLadder");
  buttonLadder =
  ...
  */
  ...
  //	//TheShell->registerWithAnimateManager(nullptr,
  //WIN_ANIMATION_SLIDE_LEFT, TRUE, 400);
  //	TheShell->registerWithAnimateManager(buttonBuddies,
  ...
  case GBM_SELECTED: {
    ...
    if (controlID == buttonBackID) {
      ...
      **/
      ...
      if ((TheDisplay->getWidth() != DEFAULT_DISPLAY_WIDTH ||
           TheDisplay->getHeight() != DEFAULT_DISPLAY_HEIGHT) &&
