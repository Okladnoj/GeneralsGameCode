...
//		m_name.str(),sourceObj->getTemplate()->getName().str(),victimObj?victimObj->getTemplate()->getName().str():"null"));
	...
	if( !ignoreRanges && !isLeechRangeWeapon() )
	{
		...
		{
			//DEBUG_ASSERTCRASH(distSqr < 5*5 || distSqr < attackRangeSqr*1.2f, ("*** victim is out of range (%f vs %f) of this weapon -- why did we attempt to fire?",WWMath::SqrtOrigin(distSqr),WWMath::SqrtOrigin(attackRangeSqr)));
...
#endif
		{
			DEBUG_ASSERTCRASH(distSqr > minAttackRangeSqr*0.8f, ("*** victim is closer than min attack range (%f vs %f) of this weapon -- why did we attempt to fire?",WWMath::SqrtOrigin(distSqr),WWMath::SqrtOrigin(minAttackRangeSqr)));
