	...
	if( m_startAttackFrame != 0 && m_startAttackFrame <= now )
	{
		...
		if ( isFiring )
		{
			...
			if( !m_manualTargetMode && !m_scriptedWaypointMode )
			{
				...
				dotProduct = __min( 0.99999f, __max( -0.99999f, dotProduct ) ); //Account for numerical errors.  Also, WWMath::ACosfOrigin(-1.00000) is coming out QNAN on the superweapon general map.  Heh.
				Real angle = (Real)WWMath::ACosfOrigin( dotProduct );
